...
* 데이터의 각 layer들은 $h\times w \times d$구조로 되어있음. 이미지를 예시로 들면 $height \times width \times channel(RGB\;or\;grayscale)$로 표현됨.
* Convnet의 핵심적인 특징은 이미지의 translation에 강인함.
* 저자들의 FCN은 어떤 크기의 입력이 들어오던 상관 없고, 그 크기의 이미지를 출력해줄 수 있음.
(여기서 내 생각을 추가하자면, 기존의 FC-layer를 사용할 경우엔 데이터의 수가 고정되어 있어야 했기 때문에, 
이미지의 크기가 항상 고정된 상태에서만 이용가능했다. 그러나 FCN은 다양한 크기의 입력 이미지들을 활용할 수 있게 해준다.)

* fully connected layer들은 input region 전체를 커버하는 kernel로 convolution으로 보여질 수 있다.
* 이런 과정은 다음 이미지에서 간략히 볼 수 있다.
* 
| ![FC 바꾸기](/assets/images/다양한 공부/논문/FCN/figure2.png) |
|:--: |
| * fully connected layer -> convolution layer * |
* 속도적인 측면에서도 fully connected를 쓰는 것보다 convolution을 쓰는 것이 더 빠르다.

* semantic segmentation 같은 문제를 해결할 땐, fugre 2에서 볼 수 있는 spatial output map을 갖는게 좋다.
* 그리고 forward,backward 계산시에 효율적이라고 함.
