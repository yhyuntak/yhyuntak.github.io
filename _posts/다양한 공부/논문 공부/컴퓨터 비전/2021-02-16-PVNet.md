...
이렇게 학습된 unit vector feature map을 통해서 RANSAC 기반 voting으로 2D Keypoint들을 추출하고, PnP 알고리즘을 사용해서 R,t를 찾는 것이다. 
...
우리는 이미 3D 모델의 pose와 키포인트,semantic label을 알고, 가상의 데이터로 2D projection 키포인트들도 안다고 가정한 후, 
CNN네트워크를 이미지만 보더라도 unit vector랑 sematic 라벨만 생성하게 하는게 목표다. 그래서 키포인트들은 이미지의 일부분만 보더라도 고정된다.

hypothesis들의 confidence를 구한다. 이것은 나중에 마할노비스 거리에서 평균과 분산을 구할 때, outlier를 제거하는 효과를 갖는다. 
그래서 내부적인 알고리즘으로 keypoint를 구한 후, 이것들로 PnP알고리즘을 돌려서 R,t를 구하는게 목표다.